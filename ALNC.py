...
def analyze_data(data):
    ...
    for interface in interfaces:
        ...
        if state_match:
            ...
            if any(x in status for x in ['已连接', '已连线', '关联', 'connected', 'associated']):
...
def silent_login():
    try:
        ...
        if "script" in response.text:
            redirect_url = re.search(r"window\.location\.href='(.*?)'", response.text).group(1)
...
def main(self=None):
    ...
    while True:
        ...
        if raw_data:
            ...
            toaster.show_toast("网络状态", "已连接: " + ssid if ssid else "未连接", duration=0.5)
            if ssid == "SDTBU-STU":
                if network_test():
                    print("网络可用，程序进入休眠状态...")
                    toaster.show_toast("网络状态", "网络可用✅ ，进入休眠模式", duration=1)
                    ...
                    while True:
                        ...
                        if current_raw_data:
                            ...
                            if current_ssid != last_connected_ssid:
                                ...
                                toaster.show_toast("网络状态", "WiFi连接状态已更改，程序唤醒中", duration=1)
                                last_connected_ssid = current_ssid
                                break
                else:
                    print("网络不可用，进行自动登录...")
                    toaster.show_toast("网络状态", "网络不可用，进行自动登录...", duration=0.5)
                    silent_login()
            else:
